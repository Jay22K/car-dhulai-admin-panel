...
module.exports.SearchProduct = async (req, res) => {
    try {
        const search = req.body.search;
        const packages = await ProductModel.find({ title: { $regex: search, $options: 'i' }, is_active: 1 }).populate('serviceId', 'name image');
        if (packages) {
            const packageData = packages.map(package => ({
                "id": package._id,
                "title": package.title,
...
// Get All Trending (Randomly)
module.exports.GetTrendingPackages = async (req, res) => {
    try {
        const packages = await Package.find({ is_active: 1 }).sort({
            createdAt: -1
        }).limit(8)
...
module.exports.GetAllProducts = async (req, res) => {
    try {
        const packages = await Products.find({ is_active: 1 }).populate('serviceId', 'name image');
        if (packages.length > 0) {
...
module.exports.GetProductDetails = async (req, res) => {
    try {
        ...
        const packageId = req.body.packageId;
        const packages = await Products.find({ _id: packageId }).populate('serviceId', 'name image');
        // console.log(packages);
        // fetch reviews of the package
        const reviews = await PackageReview.find({ packageId: packageId }).populate('userId', 'name image');
        totalRating = 0;
...
module.exports.BookVehicle = async (req, res) => {
    try {
        const { user, address, service, package, vehicle, bookingDate, bookingTime, price, paymentMode, transactionId, paymentStatus, orderStatus, subTotal, coupon_code, coupon_type, coupon_amount, VAT, Total, products, total_product_amount } = req.body;
        console.log(req.body);
        ...
        if (saveBooking) {
            ...
            const bookedSlot = new SlotBooking({
                products: products,
                total_product_amount: total_product_amount,
                userId: user.userId, vehicle, service, bookingId: saveBooking._id, bookingDate: formattedBookingDate, bookingTime
            });
